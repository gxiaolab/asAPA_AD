...
DIR_OUT=${DIR_CONC}/STEP6_CAUSAL/v2
...
echo "--- $(date) --- Get Causal ---"
	...
	si_Controln=$(ls $si_indir/${tissue}/Control/chr3* | wc -l)
        ...
        python2 ./get.causal.v2.test.py -i $annoI -e $annoE -r $indir -t $tissue_Control -p $pval -n $minPt5Control -o $outf_Control.min5.$minPt5Control.txt  
        python2 ./get.causal.v2.test.py -i $annoI -e $annoE -r $indir -t $tissue_AD -p $pval -n $minPt5AD -o $outf_AD.min5.$minPt5AD.txt

        python2 ./get.causal.v2.test.py -i $annoI -e $annoE -r $indir -t $tissue_Control -p $pval -n $minPt10Control -o $outf_Control.min10.$minPt10Control.txt  
        python2 ./get.causal.v2.test.py -i $annoI -e $annoE -r $indir -t $tissue_AD -p $pval -n $minPt10AD -o $outf_AD.min10.$minPt10AD.txt

        python2 ./get.causal.v2.test.py -i $annoI -e $annoE -r $indir -t $tissue_Control -p $pval -n $minPt20Control -o $outf_Control.min20.$minPt20Control.txt  
        python2 ./get.causal.v2.test.py -i $annoI -e $annoE -r $indir -t $tissue_AD -p $pval -n $minPt20AD -o $outf_AD.min20.$minPt20AD.txt

	python2 ./get.causal.v2.py -i $annoI -e $annoE -r $indir -t $tissue -p $pval -n 10 -o $outf.n10.txt
