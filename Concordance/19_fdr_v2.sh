...
do
	...
	do
		outdir1=${DIR_OUT}/v2/Control.filtered2.fisherP.$method.$pthresh
		outdir2=${DIR_OUT}/v2/AD.filtered2.fisherP.$method.$pthresh
		...
		R CMD BATCH --no-save '--args indir="'$indir1'" padjmethod="'$method'" pthresh='$pthresh' outdir="'$outf_Control'"' ./scripts/p.adjust.R log/fisherP.adjust_r/.si.detect.non-causal.v1.Control
		R CMD BATCH --no-save '--args indir="'$indir2'" padjmethod="'$method'" pthresh='$pthresh' outdir="'$outf_AD'"' ./scripts/p.adjust.R log/fisherP.adjust_r/.si.detect.non-causal.v1.AD
	done
