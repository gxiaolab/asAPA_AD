...
# 11/8/2023: recalculate dist for apa => no downstream exon + all candidates are in 3'UTR already
# 			- do we even need to calculate distance since we filter for candidates in 3'UTR already?
# 			- how to associate a transcript with an event? do we need to use another annotation?
...
def main(argv):
	...
	out = open(opts.o,'w')
	out.write('causalCandidate\tsource\tnt\texon\ttagSNV\ttotalIndiv\tRR\tRV\tVV\tpeakSi\tzScore\tpValue\tnComp\tpeakSiMean\tpeakSiStdev\tpeakSiN\tpeakSiR\n')
	#AS region is exon
