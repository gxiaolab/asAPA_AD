...
def main(argv):
	...
	for ff in glob.glob('{}/{}/*.txt'.format(opts.r,opts.t)): #ff: 1 chrm in 1 tissue at a time
 	# for ff in glob.glob('{}/{}*.txt'.format(opts.r,opts.t)):
		with open(ff) as f:
			for ll in f:
				if not ll.startswith('causalCandidate'):
					...
					if len(set([cstrd,estrd,tstrd])) == 1: #and l[5] not in l[6:9]: #strands agree AND not everyone has the same GT!! ==> 1/9/2017: it is okay to have same gt!!!
						...
						if len(means)>1:
          						maxsi, minsi = np.array(peakSiNs).argsort()[-2:]
							if pvals.count("NA;NA") != len(means)-1: 
								# print(pvals)
								continue
           				try:
		...
		for (cand,exon) in li.iterkeys():
			...
			if len([x for x in pv1[(cand,exon)].values() if x > PV1 or np.isnan(x)]) == 0: #all tag snvs have sig pv for the single peak (si away from 1)
				if len([x for x in pv0[(cand,exon)].values() if x > PV0 or np.isnan(x)]) == 0: #all tag snvs have sig pv for the single peak (si away from 0)
	...
	out = open(opts.o,'w')
	out.write('causalCandidate\tsource\tnt\texon\ttagSNV\ttotalIndiv\tRR\tRV\tVV\tpeakSi\tzScore\tpValue\tnComp\tpeakSiMean\tpeakSiStdev\tpeakSiN\tpeakSiR\tp0\tp1\n')
	...
	for (cand,exon) in res.iterkeys():
		for tag,l in res[(cand,exon)].iteritems():
			out.write('{}\t{}\t{}\n'.format(l, pv0[(cand,exon)][tag],pv1[(cand,exon)][tag]))
	out.close()
