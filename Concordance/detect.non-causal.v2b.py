		...
		with open(opts.r) as f:
			for ll in f:
				...
				try:
					...
					Sis = map(float,l[13].split('|'))
					maxgmm, mingmm = np.array(peakSiNs).argsort()[-2:]
					thresh[(l[0],l[3],l[4])] = (Sis[maxgmm]+Sis[mingmm])/2
...
#	hetgood,hetbad,homgood,hombad = 0, 0, 0, 0
	for ff in glob.glob('{}/{}*'.format(opts.i,opts.s)):
		with open(ff) as f:
			for l in f:
...
#				data[(l[0],l[3])][gt].append(float(l[-1]))
	...
	with open(opts.o, 'w') as out:
		out.write('causalCandidate\tsource\tnt\texon\ttagSNV\ttotalIndiv\tRR\tRV\tVV\tpeakSi\tzScore\tpValue\tnComp\tpeakSiMean\tpeakSiStdev\tpeakSiN\tpeakSiR\tp0\tp1\tfisherP\n')
