...
def main(argv):
	...
	for ff in glob.glob('{}/{}/*.txt'.format(opts.r,opts.t)): #ff: 1 chrm in 1 tissue at a time
		with open(ff) as f:
			for ll in f:
				if not ll.startswith('causalCandidate'):
					l = ll.split('\t')
					if int(l[5]) == int(l[6]) or int(l[5]) == int(l[8]): continue #filter out candidate snp if it is homozygous in all individuals
					cstrd = l[0][-1]
					estrd = l[3][-1]
					tstrd = l[4][-1]
					if len(set([cstrd,estrd,tstrd])) == 1: #and l[5] not in l[6:9]: #strands agree AND not everyone has the same GT!! ==> 1/9/2017: it is okay to have same gt!!!
						NCOMP = int(l[12])
						#3/24/2018: instead of just focusing on ncomp of 2, consider all cases as long as it's >=2 
						#		(ncomp = 1 was considered in v1 & v2b)
						if NCOMP == 1: continue
						#test using the largest 2 GMMs
						peakSiNs = map(int,l[15].split('|'))
						maxgmm = np.argmax(peakSiNs)
						mingmm = np.argmin(peakSiNs)
						if maxgmm == mingmm:
          						maxgmm = mingmm+1
						if peakSiNs[maxgmm] + peakSiNs[mingmm] < 0.9*sum(peakSiNs): continue #0.9 was selected to include our gmas evegas case!
						nn[(l[0],l[3])][l[4]] = int(l[-12])
						pvals = l[-6].split('|')
						means = map(float,l[13].split('|'))
						if means[maxgmm] > means[mingmm]:
							maxsi = maxgmm
							minsi = mingmm
						else:
							maxsi = mingmm
		...
		for (cand,exon) in li.iterkeys():
			...
			if len([x for x in pv1[(cand,exon)].values() if x <= PV1]) == 0: #no tag snvs have sig pv for x_large peak (si away from 1)
     					if len([x for x in pv0[(cand,exon)].values() if x > PV0 or np.isnan(x)]) == 0: #all tag snvs have sig pv for x_small peak (si away from 0)
							if 1:
	...
	for (cand,exon) in res.iterkeys():
		for tag,l in res[(cand,exon)].iteritems():
			out.write('{}\t{}\t{}\t{}\t{}\t{}\n'.format(l,'test','test','test',pv0[(cand,exon)][tag],pv1[(cand,exon)][tag]))
